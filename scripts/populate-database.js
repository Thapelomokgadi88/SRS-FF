...
async function populateDatabase() {
  ...
  try {
    ...
    for (const faculty of facultyData) {
      ...
      for (const template of templates) {
        programmes.push({
          ...
          total_credits: template.credits,
          duration_years: template.duration
        });
        programmeCounter++;
      }
    }
    ...
    console.log(`✓ Inserted ${programmeData.length} programmes`);
    ...
    for (const programme of programmeData) {
      const faculty = facultyData.find(f => f.faculty_id === programme.faculty_id);
    ...
    for (let i = 0; i < allModules.length; i += batchSize) {
      ...
      insertedModules += moduleData.length;
    ...
    for (let i = 1; i <= 500; i++) {
      ...
      const programme = faker.helpers.arrayElement(programmeData);
      
      students.push({
        student_no: `STU${currentYear}${i.toString().padStart(4, '0')}`,
        first_name: name.firstName,
        last_name: name.lastName,
        id_number: generateSAIdNumber(birthYear),
        ...
        pin_hash: '$2b$10$' + faker.string.alphanumeric(53) // Mock bcrypt hash
      });
    }
    ...
    for (let i = 0; i < students.length; i += batchSize) {
      ...
      insertedStudents += studentData.length;
    ...
    for (const student of allStudentsData.slice(0, 100)) { // Enrol first 100 students
      const studentProgramme = programmeData.find(p => p.programme_id === student.programme_id);
      const programmeModules = allModulesData.filter(m => m.programme_id === student.programme_id);
      
      // Enrol in modules for their current year
      const currentStudentYear = Math.min(
        currentYear - student.intake_year + 1,
        studentProgramme.duration_years
      );
      
      const yearModules = programmeModules.filter(m => m.year_level <= currentStudentYear);
      ...
      for (const module of yearModules.slice(0, 8)) { // Limit to 8 modules per student
        enrolments.push({
          ...
          semester: module.semester_offered,
    ...
    if (enrolments.length > 0) {
      ...
      console.log(`✓ Inserted ${enrolmentData.length} enrolments`);
    }
    
    // 7. Generate assessment types for some modules
    console.log('Generating assessment types...');
    const assessmentTypes = [];
    
    for (const module of allModulesData.slice(0, 50)) { // Add assessments for first 50 modules
      const assessments = [
        { name: 'Assignment', weight_pct: 20 },
        { name: 'Test', weight_pct: 30 },
        { name: 'Final Exam', weight_pct: 50 }
      ];
    ...
    if (assessmentTypes.length > 0) {
      ...
      console.log(`✓ Inserted ${assessmentData.length} assessment types`);
    }
    
    console.log('\n🎉 Database population completed successfully!');
    console.log('\nSummary:');
    console.log(`- Faculties: ${facultyData.length}`);
    console.log(`- Programmes: ${programmeData.length}`);
